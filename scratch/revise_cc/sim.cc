...
ChangeBW(NetDeviceContainer *dev) {
    ...
    if (flag==0){
    dev->Get(0)-> SetAttribute ("DataRate", StringValue ("70Mbps"));
    dev->Get(1)-> SetAttribute ("DataRate", StringValue ("70Mbps"));
    outfile1 << "BW changed to 20 " <<flag<< "  Time: "<<Simulator::Now()/1000000000 <<std::endl;
    Simulator::Schedule(Seconds(40), &ChangeBW, dev);
    }
    flag++;
}
...
int main (int argc, char *argv[])
{
  ...
  double error_p = 0.0;
  std::string bottleneck_bandwidth = "30Mbps";//"130Mbps";//"2Mbps";//
  ...
  */
  ...
  bottleneckRouter->Get(0)->SetAttribute  ("DataRate", StringValue ("30Mbps"));
  bottleneckRouter->Get(1)->SetAttribute  ("DataRate", StringValue ("30Mbps"));
  ...
  uint32_t size = static_cast<uint32_t>((std::min (access_b, bottle_b).GetBitRate () / 8) *
    ((access_d + bottle_d + access_d) * 2).GetSeconds () * 1); // switch queue size = 5 * BDP
  ...
  double start_t[nLeaf]={10};
  double stop_t[nLeaf]={15};
  ...
  /////
  //Simulator::Schedule(Seconds(40), &ChangeBW, bottleneckRouter); //change pointToPoint
