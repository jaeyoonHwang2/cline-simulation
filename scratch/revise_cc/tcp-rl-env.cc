...
TcpGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  m_new_pacingRate = box->GetValue(0);
...
void
TcpEventGymEnv::TxPktTrace(Ptr<const Packet>, const TcpHeader& head, Ptr<const TcpSocketBase>)
{
  NS_LOG_FUNCTION (this);
  txSeqNum = (&head)->GetSequenceNumber().GetValue();
  rxSeqNum = (&head)->GetSequenceNumber().GetValue();
}
...
TcpEventGymEnv::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
  ...
  if (m_tcb->m_cWnd * 717 / 1024 > unsigned(180)){
    m_new_ssThresh = m_tcb->m_cWnd * 717 / 1024;
  }
  else {
    m_new_ssThresh = 180;
  }
...
TcpEventGymEnv::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  //tcb->m_currentPacingRate = m_new_pacingRate;
  tcb->m_maxPacingRate = m_new_pacingRate;
}
...
TcpTimeStepGymEnv::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  // action
  tcb->m_cWnd = m_new_cWnd;
  //tcb->m_currentPacingRate = m_new_ssThresh;
  tcb->m_maxPacingRate = m_new_ssThresh;
}
