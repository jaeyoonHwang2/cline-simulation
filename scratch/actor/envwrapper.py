...
class TcpEnvWrapper(TcpEventBased):
    ...
    def network_monitor_per_ack(self, obs, iteration, sim_time_sec):
        self.obs = obs
        self.Uuid = obs[0] - 1
        self.ack_count += 1
        if not (self.obs[11]):
        ...
        return self.srtt_ms * constants.MS_TO_SEC, self.min_rtt_ms * 1000
    ...
    def srtt_monitor(self):
        if self.srtt_ms:
            ...
            self.srtt_ms = self.obs[9] * constants.US_TO_MS
    ...
    def print_network_status(self):
        for i in range(self.interval_cnt):
            total_running_sec =  (self.timestamp + 1 + i) * constants.TIMESTAMP_UNIT_US * constants.US_TO_SEC + self.iteration * self.sim_time_sec
            print(self.Uuid, total_running_sec, "throughput", self.throughput)
            print(self.Uuid, total_running_sec , "loss_rate", self.loss_rate)
            print(self.Uuid, total_running_sec , "srtt", self.srtt_ms)
    ...
    def obs_to_state(self):
        ...
        self.z0[3] = self.obs[2] * constants.US_TO_SEC  # time after simulation started
        # self.z0[4] = 0
        self.z0[5] = self.obs[5]  # cwnd_u
        self.z0[6] = self.obs[5] * 8 / (self.srtt_ms * constants.MS_TO_SEC)  # pacing_rate
    ...
    def print_orca_status(self):
        total_running_sec =  self.obs[2] * constants.US_TO_SEC + self.iteration * self.sim_time_sec
        print(self.Uuid, total_running_sec, "reward", self.reward, "epochs", self.epoch_for_whole)
        print(self.Uuid, total_running_sec, "return", self.ret, "epochs", self.epoch_for_whole)
        pass
