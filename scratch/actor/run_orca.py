...
if config.mode == 'inference':
    ...
    local_job_device = '/job:%s/task:%d' % ('actor', config.task)
    shared_job_device = '/job:learner/task:0'
    is_learner = False
    global_variable_device = shared_job_device + '/cpu'
    def is_actor_fn(i):
        return i == config.task
    cluster = tf.train.ClusterSpec({
            'actor': ['localhost:%d' % (8001 + i) for i in range(params.dict['num_actors'])],
            'learner': ['localhost:8000']
        })
    server = tf.train.Server(cluster, job_name='actor', task_index=config.task)
...
with tf.Graph().as_default(), tf.device(local_job_device + '/cpu'):
    ...
    actor_op = []
    tfeventdir = os.path.join(base_path, params.dict['logdir'], 'actor'+str(0))
    ...
    while True:  # training -> lots of episodes
        ...
        for i in range(num_senders):
            ...
            a_init = agent.get_action(env[i].s0_rec_buffer, not is_eval)
        ...
        while True:  # inference -> single episode
            ...
            if env[Uuid].is_this_new_mtp():  # new MTP
                ...
                if not slow_start[Uuid]:  # no slow start
                    ...
                    a1 = agent.get_action(env[Uuid].s1_rec_buffer, not is_eval)
