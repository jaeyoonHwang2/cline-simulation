...
int main (int argc, char *argv[])
{
  ...
  double tcpEnvTimeStep = 0.000001;
  ...
  std::string access_bandwidth = "2000Mbps";//"10Mbps";//
  ...
  */
  ...
  //bottleneckRouter->Get(0)->SetAttribute  ("DataRate", StringValue ("2Mbps"));
  //bottleneckRouter->Get(1)->SetAttribute  ("DataRate", StringValue ("2Mbps"));
  ...
  uint32_t size = static_cast<uint32_t>((std::min (access_b, bottle_b).GetBitRate () / 8) *
    ((access_d + bottle_d + access_d) * 2).GetSeconds () * 2); // switch queue size = 5 * BDP
  ...
  //Simulator::Schedule(Seconds(10), &DecreaseDataRate, bottleneckRouter); //change pointToPoint
  Simulator::Run ();
